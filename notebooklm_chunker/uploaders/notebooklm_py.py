...
class NotebookLMPyUploader:
    def list_notebooks(self) -> list[dict[str, str | None]]:
        return asyncio.run(self._list_notebooks_async())
    ...
    def list_artifacts(self, notebook_id: str) -> list[dict[str, Any]]:
        return asyncio.run(self._list_artifacts_async(notebook_id))

    def delete_artifacts(self, notebook_id: str, artifact_ids: list[str]) -> None:
        asyncio.run(self._delete_artifacts_async(notebook_id, artifact_ids))

    async def _list_artifacts_async(self, notebook_id: str) -> list[dict[str, Any]]:
        ...
        async with await client_class.from_storage() as client:
            ...
            for artifact in artifacts or []:
                normalized.append(
                    {
                        "id": _read_attr(artifact, "id"),
                        "kind": _read_attr(artifact, "kind"),
                        "title": _read_attr(artifact, "title"),
                        "status": _read_attr(artifact, "status"),
                        "updated_at": _read_attr(artifact, "updated_at"),
                        "created_at": _read_attr(artifact, "created_at"),
                    }
                )
            return normalized

    async def _delete_artifacts_async(self, notebook_id: str, artifact_ids: list[str]) -> None:
        client_class = _load_notebooklm_client_class()
        async with await client_class.from_storage() as client:
    ...
    async def _upload_directory_async(
        ...
        *,
        ...
        client_class = _load_notebooklm_client_class()
        run_state = (
            RunStateStore.load(markdown_files[0].parent / RUN_STATE_BASENAME)
            if resume and markdown_files
            else RunStateStore(markdown_files[0].parent / RUN_STATE_BASENAME)
            if markdown_files
            else None
        )
        try:
    ...
    async def _list_notebooks_async(self) -> list[dict[str, str | None]]:
        client_class = _load_notebooklm_client_class()
        try:
            async with await client_class.from_storage() as client:
                ...
                return [
                    {
                        "id": _read_attr(notebook, "id"),
                        "title": _read_attr(notebook, "title"),
                    }
                    for notebook in notebooks
                ]
        except UploadError:
            raise
        except Exception as exc:  # pragma: no cover - live client failure
            raise UploadError(
                "notebooklm-py notebook listing failed. Make sure `notebooklm login` completed successfully."
            ) from exc
        ...
        if studio_config.per_chunk:
            ...
            for uploaded in uploaded_sources:
                if uploaded.source_id is None:
                    continue
                if requested_source_ids and uploaded.source_id not in requested_source_ids:
                    continue
                jobs.append(
                    (
                        studio_name,
